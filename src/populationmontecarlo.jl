...
Recover PMC fitting result from a `checkpoint.jld2`-file and assign the content to `f`. <br>
Do not rely on this for long-term storage. 
Checkpoint files are only a safeguard for short-term recovery (e.g. because your session was involuntarily interrupted). 
"""
function load_pmcres_from_checkpoint!(f::ModelFit, path_to_checkpoint::String)
    pmcres = load_pmcres_from_checkpoint(path_to_checkpoint)
