...
The generated "fitting simulator" 
    ...
    - Defines a second method for the fitting_simulator which dispatches to the original `simulator` function
...
function generate_fitting_simulator(
    ...
    fitting_simulator(p::ComponentVector; kwargs...) = simulator(p; kwargs...)
...
function generate_fitting_simulator(
    ...
    # matching parameter labels to indices
    pfit_labels = ComponentArrays.labels(pfit[1])
    idxs = [findfirst(x -> x == l, pfit_labels) for l in intguess]

    function fitting_simulator(pvec::Vector{R}; kwargs...) where R <: Real
        
        psim = pfit[threadid()] # pick the parameter copy for the current thread
        psim[idxs] = pvec # assign parameters
