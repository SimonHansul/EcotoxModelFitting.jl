...
@testset "Constructing PMC backend" begin
    global f = PMCBackend(
        ...
        savedir = joinpath("test", "DEB", "test01_growth_only")
    )
...
@testset "Fitting to growth data using PMC" begin
    ...
    if isdir("test/DEB/test01_growth_only")
        rm("test/DEB/test01_growth_only", recursive = true)
    end
    ...
    begin # running the calibration
        ...
        function plot_pmc_loss(pmcres)
            ...
            plt = plot(
                    ...
                    marker = true, lw = 1.5, 
                    xlabel = "PMC step", ylabel = "Distance", 
                    label = "Minimum"
                    )
            plot!(
    ...
    @test isfile("test/DEB/test01_growth_only/checkpoint.jld2")
    @test isfile("test/DEB/test01_growth_only/pmc_samples.csv")
    @test isfile("test/DEB/test01_growth_only/pmc_settings.csv")
    @test isfile("test/DEB/test01_growth_only/posterior_summary.csv")
    @test isfile("test/DEB/test01_growth_only/priors.csv")
    #@test isfile("test/DEB/test01_growth_only/retrodictions.png")
    

    # FIXME: retrodictions.png is not present
    # remove output files
    #rm("test/DEB/test01_growth_only", recursive = true)
end
